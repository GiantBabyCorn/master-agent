...
class CodexProvider:
    ...
    def launch_task(self, request: ProviderTaskRequest) -> ProviderTaskResult:
        ...
        args = [settings.codex_cli_command, "exec", request.prompt]
        try:
            proc = subprocess.run(
                args,
                capture_output=True,
    ...
    def get_task(self, external_run_id: str) -> ProviderTaskResult:
        return ProviderTaskResult(success=False, output="", error="codex_cli does not support get_task")

    def followup_task(self, external_run_id: str, prompt: str) -> ProviderTaskResult:
        return ProviderTaskResult(success=False, output="", error="codex_cli does not support followup_task")

    def stop_task(self, external_run_id: str) -> ProviderTaskResult:
        return ProviderTaskResult(success=False, output="", error="codex_cli does not support stop_task")
